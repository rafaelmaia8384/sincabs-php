<?php
	...
	if (!empty($_POST['id_usuario']) 		&&
		...
		$date_time_max = DBEscape($_POST['date_time_max']);
		...
		if (UsuarioVerificado($id_usuario, $online_hash)) {
			...
			$result = DBRead('tb_suspeitos', "WHERE suspeito_excluido = 0 AND data_registro < '$date_time_max' ORDER BY data_registro DESC LIMIT {$limit}, {$search_limit}", 'img_principal, img_busca, id_suspeito, nome_alcunha, nome_completo, data_registro, areas_de_atuacao');
			...
			if (is_array($result)) {
				...
				Sucesso('Suspeitos encontrados.', $result_array);
			}
			else {
